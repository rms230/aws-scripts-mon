...
Description of available options:
  ...
  --physical-mem-util         Reports physical memory utilization in percentages.
  --physical-mem-used         Reports physical memory used in megabytes.
  --physical-mem-avail        Reports available physical memory in megabytes.
  --virtual-mem-util          Reports virtual memory utilization in percentages.
  --virtual-mem-used          Reports virtual memory used in megabytes.
  --virtual-mem-avail         Reports available virtual memory in megabytes.
...
my $report_physical_mem_util;
my $report_physical_mem_used;
my $report_physical_mem_avail;

my $report_virtual_mem_util;
my $report_virtual_mem_used;
my $report_virtual_mem_avail;
...

{
  ...
  $parse_result = GetOptions(
    ...
    'physical-mem-util' => \$report_physical_mem_util,
    'physical-mem-used' => \$report_physical_mem_used,
    'physical-mem-avail' => \$report_physical_mem_avail,
    'virtual-mem-util' => \$report_virtual_mem_util,
    'virtual-mem-used' => \$report_virtual_mem_used,
    'virtual-mem-avail' => \$report_virtual_mem_avail,
...
if (!$report_physical_mem_util && !$report_physical_mem_used && !$report_physical_mem_avail
  && !$report_virtual_mem_util && !$report_virtual_mem_used && !$report_virtual_mem_avail && !$report_disk_space)
{
...
$input_ref->{'Namespace'} = "System/Windows";
...
if ($report_physical_mem_util || $report_physical_mem_used || $report_physical_mem_avail || $report_virtual_mem_util || $report_virtual_mem_used || $report_virtual_mem_avail)
{
  my %meminfo;
  my @memoryLines;
  my @memoryInfo;
  ...
  @memoryLines =  split('\n', `wmic OS get FreePhysicalMemory,TotalVirtualMemorySize,FreeVirtualMemory`);
  @memoryInfo = split('\s+', $memoryLines[1]);
  @pmemoryLines =  split('\n', `wmic ComputerSystem get TotalPhysicalMemory`);
  @pmemoryInfo = split('\s+', $pmemoryLines[1]);
  
  my $physical_mem_free = $memoryInfo[0];
  my $physical_mem_avail = $pmemoryInfo[0];
  my $virtual_mem_avail = $memoryInfo[2];
  my $virtual_memory_free = $memoryInfo[1];
  my $physical_mem_used = $physical_mem_avail - $physical_mem_free;
  my $virtual_mem_used = $virtual_mem_avail - $virtual_memory_free;
  

  if ($report_physical_mem_util) {
    my $mem_util = 0;
    $mem_util = 100 * $physical_mem_used / $physical_mem_avail if ($physical_mem_avail > 0);
    add_metric('PhysicalMemoryUtilization', 'Percent', $mem_util);
  }
  if ($report_physical_mem_used) {
    add_metric('PhysicalMemoryUsed', $mem_units, $physical_mem_used / $mem_unit_div);
  }
  if ($report_physical_mem_avail) {
    add_metric('PhysicalMemoryAvailable', $mem_units, $physical_mem_avail / $mem_unit_div);
  }
  
  if ($report_virtual_mem_util) {
    my $mem_util = 0;
    $mem_util = 100 * $virtual_mem_used / $virtual_mem_avail if ($virtual_mem_avail > 0);
    add_metric('VirtualMemoryUtilization', 'Percent', $mem_util);
  }
  if ($report_virtual_mem_used) {
    add_metric('VirtualMemoryUsed', $mem_units, $virtual_mem_used / $mem_unit_div);
  }
  if ($report_virtual_mem_avail) {
    add_metric('VirtualMemoryAvailable', $mem_units, $virtual_mem_avail / $mem_unit_div);
  } 
...
if ($report_disk_space)
{
  ...
  my $line = `wmic logicaldisk get caption,Freespace,Size | find "$df_path"`;
    ...
    my $disk_total = $fields[2]; # * KILO;
    my $disk_avail = $fields[1]; # * KILO;
    my $disk_used = $disk_total - $disk_avail;
	my $fsystem = $fields[0];
    ...
    if ($report_disk_util) {
      ...
      add_metric('DiskSpaceUtilization', 'Percent', $disk_util, $fsystem);
    }
    if ($report_disk_used) {
      add_metric('DiskSpaceUsed', $disk_units, $disk_used / $disk_unit_div, $fsystem);
    }
    if ($report_disk_avail) {
      add_metric('DiskSpaceAvailable', $disk_units, $disk_avail / $disk_unit_div, $fsystem);
    }
}
